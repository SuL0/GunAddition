...
data class AmmoType internal constructor(private val id: Int, private val durability: Short, val noteDontUseThisConstructorExceptInternalInitialization: Boolean) {
    ...
    // COMPANION OBJECT //
    companion object {
        val listOfAllAmmo = arrayListOf<AmmoType>()
        ...
        // Ammo 가져오기 //
        fun of(item: ItemStack): AmmoType? { return getAmmo(item.typeId, item.durability) }
        fun of(id: Int, durability: Short): AmmoType? { return getAmmo(id, durability) }

        private fun getAmmo(id: Int, durability: Short, initializeMode: Boolean = false): AmmoType? {
            ...
            if (initializeMode) {
                return AmmoType(id, durability, true)
            }
            return null
        }

        // 총기에 필요한 Ammo 가져오기 //
        fun getAmmoNeeded(p: Player, item: ItemStack): AmmoType? {
            val parentNode = WeaponInfoExtractor(p, item).parentNode
            return getAmmoNeeded(parentNode)
        }
        fun getAmmoNeeded(parentNode: String): AmmoType? {
            val ammoInfo = csDirector.getString("$parentNode.Ammo.Ammo_Item_ID")
