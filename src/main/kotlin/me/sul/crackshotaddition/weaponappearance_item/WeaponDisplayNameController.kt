...
class WeaponDisplayNameController : Listener {
    ...
    fun onItemChanged(e: InventoryItemChangedEvent) {
        val p = e.player
        val requiredAmmoMaterial: Material? = MainCrackShotWeaponInfoMetaManager.getAmmoItemMaterial(p)
        if (requiredAmmoMaterial != null && MainCrackShotWeaponInfoMetaManager.isSet(p)) {
            if (e.itemStack.type == requiredAmmoMaterial || e.itemStack.type == Material.AIR) {
    ...
    private fun updateMainWeaponDisplay(p: Player, displayNameType: DisplayNameType) {
        val weapon: ItemStack? = MainCrackShotWeaponInfoMetaManager.getItemStack(p)
        val configName: String = MainCrackShotWeaponInfoMetaManager.getConfigName(p)
        ...
        if (weapon == null) throw Exception()
    ...
    companion object {
        ...
        private fun makePrettyWeaponDisplayName(p: Player?, displayNameType: DisplayNameType, weapon: ItemStack, configName: String, leftAmmo: Int?, rightAmmo: Int?, possessedExtraAmmoAmt: Int?) {
            var leftAmmo = leftAmmo
            var possessedExtraAmmoAmt = possessedExtraAmmoAmt
            val hasRightAmmo = rightAmmo != null && rightAmmo >= 0
